...
export const CodeViewer = ({ code }: { code: { file: string; code: string }[] }) => {
  const [formattedCode, setFormattedCode] = React.useState<
    { file: string; code: string; error?: string }[]
  >([]);
  const [formattingErrors, setFormattingErrors] = React.useState<{ file: string; error: string }[]>(
    [],
  );
  ...
  if (!formattedCode.length) return <p className="text-center text-lg">code formatting...</p>;
  return (
