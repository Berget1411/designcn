...
export function FontPicker({
  ...
  return (
    <div className="group/picker relative">
            ...
            {groupedFonts.map((group) => (
              <PickerGroup key={group.type}>
                ...
                {group.items.map((font) => (
                  <PickerRadioItem key={font.value} value={font.value} closeOnClick={isMobile}>
                    {font.name}
                  </PickerRadioItem>
                ))}
              </PickerGroup>
      ...
      <LockButton param={param} className="absolute top-1/2 right-8 -translate-y-1/2" />
    </div>
  );
}
