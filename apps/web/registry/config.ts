...
const fontValues = bodyFonts.map((f) => f.name.replace("font-", "")) as [string, ...string[]];
...
export function getHeadingFont(fontHeading: Exclude<FontHeadingValue, "inherit">) {
  return headingFonts.find((item) => item.name === `font-heading-${fontHeading}`);
}
