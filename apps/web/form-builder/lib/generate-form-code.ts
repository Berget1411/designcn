...
export const generateFormCode = ({
  ...
  const flattenElements = (flattenedFormElements.flat() as FormElement[]).filter(
    (el) => el.fieldType === "SocialMediaButtons",
  );
...
return (
  ...
  const stringifiedStepComponents = stringifyStepComponents(formElements as FormStep[]);
