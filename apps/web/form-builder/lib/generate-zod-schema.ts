 ...
 * use in form preview
 */
export const genFormZodSchema = (formElements: FormElement[]): z.ZodObject<any> => {
  ...
  switch (fieldType) {
    case "Input":
      ...
      if (formElement.type === "tel") {
        schemaItem = "z.coerce.number({error: 'Please enter a valid phone number'})";
      }
...
export const genFormZodSchemaCode = (formElements: FormElement[]): string => {
  ...
  const schemaEntries = Object.entries(schema.shape)
    .map(([key, value]) => {
      return `"${key}": ${genFieldZodSchemaCode(formElements.find((o) => o.name === key)!)}`;
    })
    .join(",\n");
