  ...
  return (
    <div className="space-y-3">
      {list.map((element, i) => {
        ...
        if (
          formFieldTypeWithOptions.includes(element.fieldType as FormFieldTypeWithOptions) &&
    ...
    const formElements = isMS
      ? // Multi‑step form: normalize each step and its fields
        ((fields ?? []) as FormStep[]).map((step) => ({
          ...
          stepFields:
            step.stepFields
              ?.filter((o: any) => fieldTypes.includes(o.fieldType as FormFieldType))
              .map((o: any) => ({
                ...o,
          ...
          <div className="flex justify-end pt-3">
            <AnimatePresence mode="wait">
              {isLoading && (
                ...
                >
                  <Button onClick={stop} className="w-fit" size="sm" variant="secondary">
              ...
              {!isLoading && (
                <motion.div
                  ...
                  transition={{ duration: 0.3, delay: 0.1 }}
                >
                  <Button onClick={handleGenerate} className="w-fit" size="sm" disabled={!prompt}>
                    Generate <ArrowUp />
                  </Button>
                </motion.div>
              )}
        ...
        >
          <div className="border rounded-4xl squircle md:p-6 p-4 mb-4 md:mb-8 bg-muted/30">
            {isMS ? (
              ...
              >
                <RenderFormWhileStreaming list={fields as FormElement[]} form={form} />
              </form>
            )}
          </div>
        </ErrorBoundary>
      )}
      {error && <div className="text-destructive-foreground pt-3 text-center">{error.message}</div>}
      ...
      <div hidden={!!fields || !!error?.message}>
        ...
        <div className="flex gap-2 pt-1 w-full justify-start flex-wrap">
          {promptExamples.map((o) => (
            <Button variant="outline" key={o.prompt} onClick={() => setPrompt(o.prompt)}>
              {o.label}
            </Button>
          ))}
        </div>
