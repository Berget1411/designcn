...
export function MultiStepFormPreview({
  ...
  const { currentStep, isLastStep, goToNext, goToPrevious, isFirstStep, goToFirstStep } =
    useMultiStepForm({
  ...
  return (
    <div className="flex flex-col gap-8 pt-3">
      <div className={cn("flex", direction === "vertical" ? "gap-8" : "flex-col gap-8")}>
        <Stepper value={currentStep} orientation={direction}>
          {steps.map((_, index) => {
            const stepNumber = index + 1;
            const isLast = stepNumber === steps.length;
            return (
              <StepperItem key={stepNumber} step={stepNumber} className="not-last:flex-1">
                <StepperTrigger>
                  <StepperIndicator />
                </StepperTrigger>
      ...
      <div className="w-full pt-3 flex items-center justify-end gap-3">
        ...
        {isLastStep ? (
          ...
          <Button size="sm" type="button" variant="secondary" onClick={goToNext}>
            Next
            <ChevronRight />
          </Button>
        )}
