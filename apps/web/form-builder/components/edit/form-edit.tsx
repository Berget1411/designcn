...
const EditFormItem = (props: EditFormItemProps) => {
  ...
  let DisplayName =
    "static" in element && element.static ? element.content : element.label || element.name;
      ...
      <div className="flex items-center justify-between px-2">
        <div className="flex items-center justify-start gap-1 size-full">
          <div className="size-6 grid place-items-center">
            {Icon && <Icon className="size-4 dark:text-muted-foreground text-muted-foreground " />}
          </div>
...
const useLayoutTracker = () => {
  ...
  const [isLayoutTransitioning, setIsLayoutTransitioning] = React.useState(false);
          ...
          >
            {step.stepFields.flat().map((element, fieldIndex) => {
              return (
                ...
                >
                  <EditFormItem fieldIndex={fieldIndex} element={element} stepIndex={stepIndex} />
                </Reorder.Item>
              );
            })}
          </Reorder.Group>
