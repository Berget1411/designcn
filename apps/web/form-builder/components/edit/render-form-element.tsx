...
export const RenderFormElement = ({
  ...
  switch (formElement.fieldType) {
      ...
      return (
        <Controller
          ...
          render={({ field, fieldState }) => {
            ...
            const value = Array.isArray(field.value) ? field.value : [field.value ?? max / 2];
            return (
              <Field data-invalid={fieldState.invalid}>
      ...
      return (
        <Controller
          ...
          render={({ field, fieldState }) => {
            ...
            const singleValue =
              stringValue && optionValues.includes(stringValue) ? stringValue : undefined;
            return (
                  ...
                  <ToggleGroup
                    ...
                    value={
                      ...
                      [field.value]
                        .flat()
                        .map(String)
                        .filter((val) => val !== "" && optionValues.includes(val))
                    }
                    className="flex justify-start items-center w-full flex-wrap"
                  >
                    {options}
            ...
            <Field data-invalid={fieldState.invalid} className="gap-2">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    {field.value
                      ? formElement.options.find((option) => option.value === field.value)?.label
              ...
              >
                ...
                <MultiSelectContent>
                  {(formElement?.options || []).map((option) => {
                    if (!option.label || !option.value) return null;
                    return (
                      <MultiSelectItem key={crypto.randomUUID()} value={option.value}>
                        {option.label}
                      </MultiSelectItem>
                    );
                  })}
            ...
            <Field data-invalid={fieldState.invalid} className="gap-1 [&_p]:pb-2">
              ...
              >
                {(formElement?.options || []).map(({ label, value }) => (
                  <div key={crypto.randomUUID()} className="flex items-center gap-x-2">
                  ...
                  <PopoverTrigger asChild>
                    <div className="relative">
                      ...
                      >
                        ...
                        {selectedDate && (
                          <>
                            {mode === "single" && format(selectedDate, "dd MMM, yyyy")}
                            {mode === "range" && (
                              <div className="flex items-center gap-x-2">
                                {selectedDate?.from && format(selectedDate.from, "dd MMM, yyyy")}
                                {selectedDate?.from && " - "}
                                {selectedDate?.to && format(selectedDate.to, "dd MMM, yyyy")}
                              </div>
                            )}
                            {mode === "multiple" && `${selectedDate?.length} dates selected`}
                          </>
                        )}
                        {!fieldState.isDirty && <span>{formElement.placeholder}</span>}
                      </Button>
                      {fieldState.isDirty && (
                        <Button
                          type="button"
    ...
    case "SocialMediaButtons": {
      ...
      return formElement.layout === "row" ? (
        ...
        <div className="w-full items-center flex-col gap-3 flex pb-3">
          {formElement.links.map((key) => (
            <Button size="lg" key={key} type="button" variant="outline" className="w-full h-11">
              <Img src={socialLogsUrls[key].src} />
              {socialLogsUrls[key].label}
            </Button>
          ))}
          ...
          render={({ field, fieldState }) => (
            <Field data-invalid={fieldState.invalid} className="gap-1 [&_p]:pb-2">
              ...
              <TagInput
                ...
                setTags={(tags) => {
                  const newTags =
                    typeof tags === "function" ? tags(field.value ?? formElement.tags ?? []) : tags;
    ...
    case "Text": {
      ...
      if (variant === "H1") {
        return (
          <h1
            ...
            className={cn("mt-6 font-black text-3xl tracking-tight mb-1", formElement.className)}
          >
            {formElement.content}
          </h1>
        );
      ...
      if (variant === "H3") {
        return (
          <h3 className="mt-3 font-bold text-xl tracking-tight mb-1">{formElement.content}</h3>
        );
      }
      if (variant === "P") {
        return (
    ...
    case "H1":
      return (
        <h1
          key={formElement.content}
          className={cn("mt-6 font-extrabold text-3xl tracking-tight", formElement.className)}
        >
          {formElement.content}
        </h1>
      );
    case "H2":
      return <h2 className="mt-4 font-bold text-2xl tracking-tight">{formElement.content}</h2>;
    case "H3":
      return <h3 className="mt-3 font-semibold text-xl tracking-tight">{formElement.content}</h3>;
    case "P":
      return (
