...
import { CodeBlock, CodeBlockCode, CodeBlockGroup } from "@workspace/ui/components/code-block";
  ...
  return (
    <CodeBlock className="my-0 w-full">
      <CodeBlockGroup className="border-border border-b px-4 py-2">
        <div className="bg-muted py-1 px-1.5 rounded-sm text-muted-foreground text-sm">{title}</div>
...
export const JsonViewer = ({
  ...
  if (Array.isArray(json)) {
    json = (
      isMS ? flattenFormSteps(json as FormStep[]).flat() : (json as FormElementOrList[])
...
const installableShadcnComponents: Partial<Record<FormElement["fieldType"], string>> = {
  Input: "input",
  Textarea: "textarea",
  Checkbox: "checkbox",
  Select: "select",
...
const Cli = ({
    ...
    mutationFn: async () => {
      ...
      const res = await fetch(`/r/${key}`, {
        ...
        body: JSON.stringify({
          name,
          key,
          dependencies: ["react-hook-form", "zod", "@hookform/resolvers", "motion"],
  ...
  return (
    <div>
      <Button
        onClick={() => {
          if (!meta.id || !meta.name) {
            toast.error("Load or create a form first to generate a CLI command");
            return;
          }
          res.mutate();
        }}
      ...
      >
        {status === "pending" ? "Generating command..." : "Generate CLI command"}
      </Button>
      {status === "error" && (
        <div className="text-destructive text-sm text-center py-2 border border-destructive/20 rounded-md px-3">
          Failed to generate command: {res.error?.message ?? "Unknown error"}
        </div>
      )}
      {status === "success" && !data?.data?.id && (
        <div className="text-destructive text-sm text-center py-2 border border-destructive/20 rounded-md px-3">
          {data?.error ?? "Failed to generate command. Check console for details."}
        </div>
      )}
      {status === "success" && data?.data?.id && (
...
export function CodeBlockPackagesInstallation({
  ...
  return (
    <div className="w-full py-5 max-w-full">
      <h2 className="font-sembold text-start mb-2 text-foreground">Install npm packages</h2>
...
const useGenerateCode = () => {
  ...
  const processedFormElements = isMS ? flattenFormSteps(formElements as FormStep[]) : formElements;
...
export function CodePanel() {
  const formElements = useFormBuilderStore((s) => s.formElements);
  const { zodSchema, tsx, registryDependencies, dependencies, meta, isMS } = useGenerateCode();
  ...
  return (
    <div className="w-full min-w-full">
      <div className="flex justify-between items-center mb-4">
        <h2 className="font-semibold text-secondary-foreground">Install with CLI</h2>
      ...
      <Tabs defaultValue="tsx" className="w-full min-w-full pt-3">
        <div className="flex justify-between items-center">
          <h2 className="font-semibold text-secondary-foreground">Manual installation</h2>
          <TabsList>
